...
class ReportItemDecoration(context: Context, var groupBeans: ArrayList<GroupBean?>) : RecyclerView.ItemDecoration() {
    private val mPaint: Paint = Paint(Paint.ANTI_ALIAS_FLAG)

    private val mPaintText = Paint(Paint.ANTI_ALIAS_FLAG)

    private val height = Utils.dip2px(context, 32f).toInt()
    ...
    init {
        ...
        mPaint.color = Color.parseColor("#EBEBEB")
    }
    ...
    override fun onDrawOver(c: Canvas, parent: RecyclerView, state: RecyclerView.State) {
        ...
        for (i in 0 until parent.childCount) {
            ...
            val states = (parent.adapter as ReportList.Adapter).list
            val commodity = states[position]
            val headerText = "${commodity.categoryName}  ${commodity.secondaryCategoryName}"
